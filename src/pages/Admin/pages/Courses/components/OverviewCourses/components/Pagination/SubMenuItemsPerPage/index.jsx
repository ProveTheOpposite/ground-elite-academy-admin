...
const SubMenuItemsPerPage = ({
  ...
  return (
    <div
      ...
      className="absolute right-0 top-full flex w-full flex-col gap-y-2 rounded-xl border border-gray-300 bg-white p-2 text-slate-600"
    >
      {itemsPagination.map((item) => (
        <span
          onClick={handleClickChangeNumberItemsPerPage}
          key={item.id}
          className="cursor-pointer rounded-md py-0.5 text-center transition-colors hover:bg-slate-100"
        >
